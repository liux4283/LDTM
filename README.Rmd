...
-   we will incorperate the tree structural information into the modeling
...
that each component in the product $\prod_{v \in \mathcal V}$ corresponds to a interior node in the tree, which is a Dirichlet multinomial distribution based on the accumulated counts along branches of given node.
...
Tips: 1. when $\mathcal V \neq \{v_0\}$, each $p_l$ has an independet variance

1.  two components \$p\_{v_i}, p\_{v_j} \$ with in the same subtree indexed by $v \in \mathcal V$ are correlated, due to the same ancestor of $v$.
...
number of $p$ time the number of tree branches $\sum_{v\in \mathcal V} K_v$
